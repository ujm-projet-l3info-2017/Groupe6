...
public class ConversationIA
{
	...
	public String executerProchaineEtape()
	{
		...
		case AVIS:
			...
			if(b_film)
			{
				if (Reconnaissance.realisateur(messageCorrige))
					return "Ce film a ÈtÈ rÈalisÈ par "+film.realisateur();
				...
				if (Reconnaissance.acteurs(messageCorrige))
				{
					...
					if (acteurs != null)
						...
						return "Je ne me rappelle plus des acteurs de ce film, dÈsolÈ ...";
				}
				
				if (Reconnaissance.leGenre(messageCorrige))
				{
					String genre = film.genres().get(0);
					if (genre.charAt(0)=='a' || genre.charAt(0)=='h' || genre.charAt(0)=='È')
		...
		case VOULOIR_INFOS :
			...
			if (Reconnaissance.ouiOuNon(messageCorrige) == 1)
			{
				//L'utilisateur veut des infos, on passe dans l'Ètat AVIS
				prochaineEtape = Etape.AVIS;
			...
			else if (Reconnaissance.ouiOuNon(messageCorrige) == 2)
			{
				...
				prochaineEtape = Etape.DEBUT;
				return "Si tu as besoin d'autre chose, n'hÈsite pas...";
			}
			else 
			{
				return "Je peux te donner le synopsis, te dire ce que les gens en ont pensÈ, les acteurs principaux ...";
			}
